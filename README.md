...
Provides IP-based blacklisting through http:BL and allows linking to a honeypot
by using [Project Honeypot](http://www.projecthoneypot.org/).
...
Reports are available in admin/reports and are accessible to anyone with proper permissions.

There are two reports built with Views:

* http:BL Blocked Hosts
* http:BL Cleared

In order to see these reports, some level of caching must be set in admin >> config >> people >> httpbl.

If "Http:BL cache and [IP address blocking](https://backdropcms.org/project/ip_blocking) module is set, then items shown
...
Items with a status of 2 are found only in the cache for httpbl, and
...
If you're really impatient (like me), you can turn on the Verbose logging and watch Http:BL in action. This will show you how each IP is getting looked up, and what happens depending on the results (most IPs are harmless and cached as friendly). It will always query its own cache first, then only do the DNS lookup if it doesn't find anything in cache. If it finds no profile it will treat the IP as safe. If it does find a profile, but one that is not threatening, it will also treat those as safe. Otherwise, if it doesn't like what it finds it will grey or black-list that IP. Blacklisted IPs are stored in Http:BLs cache and also added to the list of Blocked IP addresses (if you chose that option and IP address Blocking module is used).
